...
function isolateDeletion(context) {
  ...
  const state = { user, candidates: [{ user: user._id, email: user.email, status: 'active' }],
    memberships: [{ user: user._id, status: 'active' }], committed: false };
  const transaction = context.mock.method(User.db, 'transaction', async (callback) => {
    ...
    const originalMemberships = state.memberships;
    try {
      ...
      state.memberships = originalMemberships;
      throw error;
    }
  });
  ...
  const memberships = { deleteMany: async (filter, options) => {
    assert.deepEqual(filter, { user: user._id });
    assert.deepEqual(options, { session });
    state.memberships = state.memberships.filter((membership) =>
      membership.user.toString() !== filter.user.toString());
    return { deletedCount: 1 };
  } };
  const cleanupMemberships = context.mock.method(memberships, 'deleteMany');
  const collection = context.mock.method(User.db, 'collection', (name) => {
    assert.ok(['candidates', 'companyusers'].includes(name));
    return name === 'candidates' ? candidates : memberships;
  });
  ...
  return { user, state, transaction, find, cleanup, cleanupMemberships, collection, remove };
}

test('deletes own account, candidates and memberships within the same transaction', async (context) => {
...
for (const scenario of ['missing', 'other', 'inactive']) {
  test(`rejects ${scenario} user without removing any data`, async (context) => {
    const { state, cleanup, cleanupMemberships, remove } = isolateDeletion(context);
    ...
    assert.equal(cleanupMemberships.mock.callCount(), 0);
...
for (const stage of ['cleanup', 'membership', 'user', 'commit']) {
  test(`propagates ${stage} failure rather than acknowledging partial deletion`, async (context) => {
    const { state, cleanup, cleanupMemberships, remove, transaction } = isolateDeletion(context);
    const failure = new Error('storage unavailable');
    if (stage === 'cleanup') cleanup.mock.mockImplementation(async () => { throw failure; });
    if (stage === 'membership') cleanupMemberships.mock.mockImplementation(async () => { throw failure; });
    ...
    if (['cleanup', 'membership'].includes(stage)) assert.equal(remove.mock.callCount(), 0);
  });
}
